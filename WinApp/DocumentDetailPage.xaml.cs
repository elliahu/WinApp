using Microsoft.UI.Xaml;
using Microsoft.UI.Xaml.Controls;
using Microsoft.UI.Xaml.Controls.Primitives;
using Microsoft.UI.Xaml.Data;
using Microsoft.UI.Xaml.Input;
using Microsoft.UI.Xaml.Media;
using Microsoft.UI.Xaml.Navigation;
using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using System.Runtime.InteropServices.WindowsRuntime;
using WinApp.Model;
using Windows.Foundation;
using Windows.Foundation.Collections;

// To learn more about WinUI, the WinUI project structure,
// and more about our project templates, see: http://aka.ms/winui-project-info.

namespace WinApp
{
    /// <summary>
    /// An empty page that can be used on its own or navigated to within a Frame.
    /// </summary>
    public sealed partial class DocumentDetailPage : Page
    {
        public static event EventHandler GoBackRequested;
        public Document Document { get; set; }
        public DocumentDetailPage()
        {
            this.InitializeComponent();
            this.DataContext = this;
        }

        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            Document = (Document)e.Parameter;

            // handle null reference
            if(Document.Name is null)
            {
                Document.Name = String.Empty;
            }
            if (Document.Description is null)
            {
                Document.Description = String.Empty;
            }
            if (Document.Content is null)
            {
                Document.Content = String.Empty;
            }

            //Breadcrumb
            breadcrumb.ItemsSource = new string[] { 
                "Home", 
                (Document.Name == String.Empty)? "New document" : "Detail - " + Document.Name
            };
            breadcrumb.ItemClicked += (sender, e) =>
            {
                GoBackRequested?.Invoke(this, EventArgs.Empty);
            };

            base.OnNavigatedTo(e);
        }

        private async void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            await StorageManager.Save(Document);
            GoBackRequested?.Invoke(this, EventArgs.Empty);
        }

        private async void DisplayDocumentDialog()
        {
            ContentDialog deleteFileDialog = new ContentDialog
            {
                Title = "Field required",
                Content = "Please, fill-in all fields.",
                CloseButtonText = "Ok"
            };

            deleteFileDialog.XamlRoot = this.Content.XamlRoot;
            ContentDialogResult result = await deleteFileDialog.ShowAsync();
        }
    }
}
